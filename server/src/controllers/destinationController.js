...
export const createDestination = async (req, res) => {
  const { 
    ...
    category, status, facilities, operational_hours 
  } = req.body;
  
  try {
    const [result] = await pool.query(
      'INSERT INTO destinations (name, location, description, image_url, price, rating, category, status, facilities, operational_hours) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        name, 
        location, 
        description, 
